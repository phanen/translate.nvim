...
describe('translate.region', function()
  ...
  it('echoes the translation to the render sink', function()
    ...
    local captured = h.exec_lua(function()
      require('translate').setup({ api = 'google', target = 'echo' })
  ...
  describe('command dispatch (use_range)', function()
    it('uses cword when marks are stale and use_range is false', function()
      h.set_lines({ 'hello world' })
      local url = h.exec_lua(function()
      ...
      h.matches('q=hello', url)
      h.eq(nil, url:match('q=world'))
    end)

    it('uses marks when use_range = true', function()
      h.set_lines({ 'hello world' })
      local url = h.exec_lua(function()
        ...
        vim.api.nvim_win_set_cursor(0, { 1, 0 })
        require('translate').region(true)
        return _G._t_url
      end)
      h.matches('q=world', url)
    end)

    it('uses visual selection when visual mode is active', function()
      h.set_lines({ 'hello world' })
      local url = h.exec_lua(function()
        vim.api.nvim_win_set_cursor(0, { 1, 0 })
        vim.cmd('normal! v')
